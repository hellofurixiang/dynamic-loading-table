...
var imgCtx = (ctp == undefined || ctp == null) ? "../images" : (ctp + "/images");
// Cookie handling
...
///////////////////////////////////////////////////////////////////////////////
var webFXTreeConfig = {
	rootIcon        : imgCtx + "/xtree/folder.png",
	openRootIcon    : imgCtx + "/xtree/openfolder.png",
	folderIcon      : imgCtx + "/xtree/folder.png",
	openFolderIcon  : imgCtx + "/xtree/openfolder.png",
	fileIcon        : imgCtx + "/xtree/file.png",
	iIcon           : imgCtx + "/xtree/I.png",
	lIcon           : imgCtx + "/xtree/L.png",
	lMinusIcon      : imgCtx + "/xtree/Lminus.png",
	lPlusIcon       : imgCtx + "/xtree/Lplus.png",
	tIcon           : imgCtx + "/xtree/T.png",
	tMinusIcon      : imgCtx + "/xtree/Tminus.png",
	tPlusIcon       : imgCtx + "/xtree/Tplus.png",
	plusIcon        : imgCtx + "/xtree/plus.png",
	minusIcon       : imgCtx + "/xtree/minus.png",
	blankIcon       : imgCtx + "/xtree/blank.png",
...
function TreePanel(panel, eventObj) {
	panel = getEle(panel);
	var obj = panel.parentNode;
	
	var screen_w = document.body.offsetWidth;
	var obj_l = getLeft(obj);
	var obj_mgr_l = obj.offsetWidth - (screen_w - obj_l);
	obj.style.marginLeft = -obj_mgr_l;
	
	eventObj = getEle(eventObj);
	eventObj.onclick = function() {
		var visibility = panel.parentNode.style.visibility;
		if(visibility == "visible"){
			panel.parentNode.style.visibility = "hidden";
		}else{
			panel.parentNode.style.visibility = "visible";
		}
	};
	...
	function bodyEvent(){
		if(window.event.srcElement == eventObj) return;
		var visibility = panel.parentNode.style.visibility;
		var _x = window.event.x;
		var _y = window.event.y;
		
		var panel_x = getLeft(panel);
		var panel_y = getTop(panel);
		
		if(visibility != "visible"){return;}
		if((_x > panel_x && _x < panel_x + panel.offsetWidth) && (_y > panel_y && _y < panel_y + panel.offsetHeight)) return;
...
/**
 * 树节点管理菜单
 * @param menus Object:link, name
 */
function XTreeNodeManageMenu(menus){
	...
	document.write("<ul id=\"__hqsoft_xtree_plug_menu\"></ul>");
	var menuPanel = document.getElementById("__hqsoft_xtree_plug_menu");
	menuPanel.style.display = "none";
	
	this.display = function(__id,__fullPath){
		menuPanel.innerHTML = "";
		//create menu
		var fixedMenus = menus.fixed;
		if(fixedMenus != null && fixedMenus.length > 0){
			for(var i=0;i<fixedMenus.length;i++){
				if(fixedMenus[i].needId && (__id == undefined || __id == null || __id == "")) continue;
				var createMenu = createFixedNodeMenu(fixedMenus[i], __id,__fullPath);
				menuPanel.appendChild(createMenu);
			}
		}
		/*
		var createMenu = createFixedNodeMenu(menus.fixed.create, "创建新的树节点", __id,__fullPath);
		menuPanel.appendChild(createMenu);
		
		if(__id != undefined && __id != ""){
			//edit menu
			var editMenu = createFixedNodeMenu(menus.fixed.edit, "修改树节点", __id,__fullPath);
			menuPanel.appendChild(editMenu);
			//del menu
			var delMenu = createFixedNodeMenu(menus.fixed.del, "删除树节点", __id,__fullPath);
			menuPanel.appendChild(delMenu);
		}
		*/
		var otherMenus = menus.other;
		if(otherMenus != null && otherMenus.length > 0){
			for(var i=0;i<otherMenus.length;i++){
				var liObj = document.createElement("li");
				var link = document.createElement("a");
				var href = otherMenus[i].link;
				if(__id == null) __id = "";
				href = href.replace("%{id}",__id);
				if(__fullPath == null) __fullPath = "";
				href = href.replace("%{fullPath}",__fullPath);
				link.innerHTML = otherMenus[i].name;
				link.href = href;
				link.target = "_parent";
	 ...
	 * 创建固定的操作菜单
	 */
	function createFixedNodeMenu(menuObj, id, fullPath){
		var liObj = document.createElement("li");
		var link = document.createElement("a");
		...
		if(typeof(href) == "string"){
			...
			href = href.replace("%{fullPath}",fullPath);
			link.href = href;
			link.target = menuObj.target == null ? "_parent" : menuObj.target;
		}else if(typeof(href) == "function"){
			link.href = "javascript:;";
			liObj.onclick = function(){href(id, fullPath);};
		}
		link.innerHTML = menuObj.name;
		liObj.appendChild(link);
		liObj.onmouseout = function(){
			this.parentNode.style.display = "none";
		};
	...
	function position(event){
		var _x = getLeft(event);
		var _w = document.body.scrollLeft + document.body.clientWidth;
		if(_x + menuPanel.offsetWidth > _w){
			//_x -= _w - (event.offsetWidth + menuPanel.offsetWidth);
			_x -= (_x + menuPanel.offsetWidth) - _x;
			_x += event.offsetWidth;
		}
		
		var _y = getTop(event); 
		var _h = 0; 
		if (typeof window.pageYOffset != 'undefined') { 
			_h = window.pageYOffset; 
		}else if (typeof document.compatMode != 'undefined' && document.compatMode != 'BackCompat') { 
			_h = document.documentElement.scrollTop; 
		}else if (typeof document.body != 'undefined') { 
			_h = document.body.scrollTop; 
		} 
		_h += document.body.clientHeight;
		
		if(_y + menuPanel.offsetHeight > _h){
			_y -= menuPanel.offsetHeight;// + event.offsetHeight;
			//_y -= event.offsetHeight;
			//alert();
		}else{
			_y += event.offsetHeight;
		}
		_y = _y < 0 ? 0 : _y;
		menuPanel.style.left = _x + "px";
		menuPanel.style.top = _y + "px";
	}
}